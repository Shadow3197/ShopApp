...
const AddProductScreen = props => {
  ...
  const submitHandler = useCallback(async () => {
    // If we are setting then were editing item if not were adding item
  ...
  useEffect(() => {props.navigation.setParams({submit: submitHandler});
}, [submitHandler]);
...
AddProductScreen.navigationOptions = navData => {
  ...
  return {
    ...
    <Item 
      title='Save' 
      iconName={Platform.OS === 
        'android' ? 'md-checkmark-circle-outline' : 'ios-checkmark-circle-outline'} 
      onPress={submitFn}
    />
  </HeaderButtons>
  };
